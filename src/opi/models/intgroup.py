...
class IntGroup(BaseModel):
    ...
    >> intgroup_ex = IntGroup(values=[1, 2, 3])
    >> print(str(intgroup_ex))
    {1 2 3}
    """
...
class IntGroupEnd(IntGroup):
    """
    Class to format a collection of integers for the ORCA inp file.
    Extends `IntGroup` by adding "end" to its string representation.
